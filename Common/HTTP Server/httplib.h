...
constexpr size_t  CPPHTTPLIB_RECV_BUFSIZ = 4096u;
...
public:
  ...
  std::shared_ptr<Response> Get(const std::string path, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress);

  std::shared_ptr<Response> Post(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress);

  std::shared_ptr<Response> Put(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress);

  std::shared_ptr<Response> Delete(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress);

  std::shared_ptr<Response> Patch(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress);

  std::shared_ptr<Response> Head(const std::string path,  const Headers& headers, const Params& params, Progress progress);

  std::shared_ptr<Response> Options(const std::string path, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress);
...
inline bool read_content_with_length(Stream &strm, uint64_t len,
  ...
  while (r < len) {
	...
	size_t sz = std::min(read_len, CPPHTTPLIB_RECV_BUFSIZ);
    auto n = strm.read(buf, sz);
    if (n <= 0) { return false; }
...
Server::process_request(Stream &strm, bool last_connection,
  ...
  if (req.method == "POST" || req.method == "PUT" || req.method == "PATCH") {
    if (!detail::read_content(strm, req, payload_max_length_, res.status,
                              Progress(), [&req](const char *buf, size_t n) {
...
inline std::shared_ptr<Response> Client::Get(const std::string path, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress)
{
	Request req;
	req.method = "GET";
	...
	if (!query.empty())
		req.path.append("?" + query);
	auto res = std::make_shared<Response>();
	return send(req, *res) ? res : nullptr;
}

inline std::shared_ptr<Response> Client::Post(const std::string path, const std::string & body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress)
{
	Request req;
	req.method = "POST";
...
inline std::shared_ptr<Response> Client::Put(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress)
{
	Request req;
	req.method = "PUT";
...
inline std::shared_ptr<Response> Client::Delete(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress)
{
	Request req;
	req.method = "DELETE";
...
inline std::shared_ptr<Response> Client::Patch(const std::string path, const std::string& body, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress)
{
	Request req;
	req.method = "PATCH";
...
inline std::shared_ptr<Response> Client::Head(const std::string path, const Headers& headers, const Params& params, Progress progress)
{
	Request req;
	req.method = "HEAD";
...
inline std::shared_ptr<Response> Client::Options(const std::string path, const Headers& headers, const Params& params, ContentReceiver content_receiver, Progress progress)
{
	Request req;
	req.method = "OPTIONS";
...
inline int SSLSocketStream::read(char *ptr, size_t size) {
  if (SSL_pending(ssl_) > 0 ||
      detail::select_read(sock_, CPPHTTPLIB_READ_TIMEOUT_SECOND,
                          CPPHTTPLIB_READ_TIMEOUT_USECOND) > 0) {
    return SSL_read(ssl_, ptr, (int)size);
  }
  return -1;
}

inline int SSLSocketStream::write(const char *ptr, size_t size) {
  return SSL_write(ssl_, ptr, (int)size);
}
